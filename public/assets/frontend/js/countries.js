var _freeShippment = 0;
var _DHLPriceShippment = 21.08;
...
function populateStates(countryElementId, stateElementId) {
    ...
    stateElement.onchange = function () {
        // console.error($(this).find(":selected").text());
...
function populateCountries(countryElementId, stateElementId) {
    ...
    if (stateElementId) {
        countryElement.onchange = function (e) {
            // console.error($(this).find(":selected").text());
            selected = $(this).find(":selected");
            ...
            if (!selected.attr("selected")) {
                ...
                if ($.inArray(country, _golfCountries) !== -1) {
                    _shippment = _freeShippment;
                    lib = parseFloat(_shippment).toFixed(2) + " KWD";
                } else {
                    _shippment = _DHLPriceShippment;
                    lib = parseFloat(_shippment).toFixed(2) + " KWD";
                }

                total = 0;
                total = _total + _shippment;
                totalLib = parseFloat(total).toFixed(2) + " KWD";

                $('#Shippment').empty().html(lib);
                $('#total').empty().html(totalLib);
                _usTotal = convert(parseFloat(total).toFixed(2));
                console.info("ustotal " + _usTotal);
                console.info("total " + total);
                $('#us_total_order').attr("value", _usTotal);
                $('#shippment_order').attr("value", _shippment);
            }
            // console.info(_shippment);
